...
public abstract class ComposerBase {
    ...
    protected void Write( object obj, bool as_object = false ) {
        if( as_object ) {
    ...
    protected void WriteKeyValue( object key, object value ) {
        this.WriteString( $"{key}:{value}" );
        this.writer!.Write( new byte[] { 13 } ); // delimiter
    }

    protected void WriteValue( object key, object value ) {
        this.WriteString( $"{key}={value}" );
        this.writer!.Write( new byte[] { 13 } ); // delimiter
    }

    protected void WriteDelimiter( object key, object value, string? delim = null ) {
    ...
    protected void WriteString( string value, bool append_terminator = true ) {
